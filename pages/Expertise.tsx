    ...
    <div className="bg-[#fcfbf7] min-h-screen animate-in fade-in duration-1000">
      ...
      <section className="py-16 md:py-24 space-y-20 md:space-y-32">
        {LAWYER_EXPERTISE.map((area, idx) => (
          <div key={area.id} className="max-w-7xl mx-auto px-6 md:px-8 lg:px-12 overflow-hidden">
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 items-center ${idx % 2 === 1 ? 'md:flex-row-reverse' : ''}`}>
              {/* Content Side */}
              <div className={`space-y-6 md:space-y-10 ${idx % 2 === 1 ? 'md:order-2' : 'md:order-1'}`}>
              ...
              {/* Visual Side */}
              <div className={`relative group ${idx % 2 === 1 ? 'md:order-1' : 'md:order-2'}`}>
                ...
                <div className="relative overflow-hidden shadow-2xl rounded-sm h-[400px] md:h-[500px] lg:h-[600px]">
                  <img 
                    src={area.image} 
                    alt={area.title} 
