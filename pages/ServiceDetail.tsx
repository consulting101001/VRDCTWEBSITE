...
const ServiceDetail: React.FC = () => {
  ...
  const backPath = location.pathname.startsWith('/expertise') ? '/expertise' : '/practice-areas';
  const backLabel = location.pathname.startsWith('/expertise') ? '← BACK TO EXPERTISE' : '← BACK TO ALL SERVICES';
    ...
    <div className="min-h-screen bg-[#fafafc] animate-in fade-in duration-1000 pb-20">
      ...
      <section className="relative h-[60vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-8 text-center">
          <Link to={backPath} className="text-gold text-[10px] uppercase tracking-[0.5em] font-black mb-8 inline-block hover:translate-x-[-10px] transition-transform">
            {backLabel}
          </Link>
          <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-serif font-bold text-white mb-6 tracking-wide drop-shadow-2xl">
            {area.title}
          </h1>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 lg:px-12 -mt-20 relative z-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-16">
          {/* Main Content Area */}
          <div className="md:col-span-2 bg-white p-8 md:p-12 lg:p-20 shadow-2xl rounded-sm border-t-8 border-gold">
            ...
            <div className="mt-20 grid grid-cols-1 md:grid-cols-2 gap-10">
              {area.features.map((feature, idx) => (
                <Link 
                  key={idx} 
                  to="/consultation"
                  state={{ practiceArea: area.id, feature: feature }}
                  className="flex items-center space-x-6 p-6 bg-[#f8fbff] border-l-4 border-gold group hover:bg-gold hover:text-black transition-all cursor-pointer"
                >
          ...
          <div className="md:col-span-1 space-y-10">
            <div className="bg-[#0c1a2b] p-8 md:p-12 text-white shadow-2xl relative overflow-hidden group">
        ...
        <div className="mt-20 text-center">
          <Link 
            to={backPath} 
            className="inline-flex items-center space-x-4 text-gold text-[11px] font-black uppercase tracking-[0.5em] border border-gold/20 px-12 py-5 hover:bg-gold hover:text-black transition-all shadow-2xl"
          >
            <span>{backLabel}</span>
          </Link>
        </div>
